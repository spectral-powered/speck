...
description = "Kotlin multi-project gradle build helper built as a gradle-plugin for use by Spectral Powered project.."
inner class ProjectInfo {
    val longName = "Speck by Spectral Powered."
    val website = "https://github.com/spectral-powered/$name"
    val vcsUrl = "$website.git"
    val scm = "scm:git:$website.git"
    val pluginImplementationClass = "$group.plugin"
                ...
                pom {
                    developers {
                        developer {
                            ...
                            email.set("kyleescobar@spectralpowered.org")
                            url.set("https://github.com/kyle-escobar")
                        }
                    }
                }
            }
...
gradlePlugin {
    plugins {
        ...
        tasks.create("generatePluginsInfo") {
            ...
            sourceSets.main {
                val targetFile = kotlin.srcDirs.first { it.name == "kotlin" }.resolve("$pkg/speck/plugin/Plugins.kt")
                outputs.file(targetFile)
                doLast {
                    @Suppress("ktlint")
                    val text = "@file:Suppress(\"MaxLineLength\", \"ktlint\")\n\n" +
                            "package ${project.group}.speck.plugin\n\n" +
                            "object Plugins {\n" +
